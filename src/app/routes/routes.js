...
module.exports = (app, passport) => {
  ...
  app.get("/logout", (req, res) => {
    ...
    let cmfsAtratar = [];
    if (bDebug)
      console.log(new Date().toISOString(), "llega el codigo", req.user.codigo);
    if (!uploadedFiles[req.user.codigo]) {
      loginModel.addLogin([
        req.user.codigo,
        req.user.nombreCentro,
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (req.file) {
        ...
        const mes1 =
          excel.substring(i, i + 4) + "-" + excel.substring(i + 4, i + 6);
        ...
        if (bDebug)
          console.log(new Date().toISOString(), "verificado el mes: ", mes1);
  ...
  app.get(
    ...
    async function (req, res) {
      ...
      if (bDebug)
        console.log(new Date().toISOString(), "# centros:", centros.length);
      res.render("centros", {
        centros,
