...
module.exports = {
  ...
  async findAll() {
    ...
    try {
      ...
      f.loger('Error en alcanceMensual.findAll():' + e.text);
      rows = e.text;
    } finally {
      if (conn) conn.end()
    }
  ...
  async resumenAlumnos() {
    ...
    try {
      ...
      if (bDebug) f.loger('resumenAlumnos() error' + e.text, 'n');
      throw err;
  ...
  async queryWhere(where) {
    ...
    try {
      ...
      if (bDebug) f.loger(`Error en queryWhere( ${where} ) con error: ${e.text}`);
    }finally{
      if (conn) conn.end();
    }
    return rows;
  ...
  async alcanceMesUpdate(alcance, data) {
    ...
    try {
      ...
      f.loger(`Error en alcanceMesUpdate: ${e.text}
      values: ${values}`);
    }finally{
      if (conn) conn.end();
    }
